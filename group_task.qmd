...
## Data Original {visibility="hidden"}
...
ggplot(first_psoe, aes(y = reorder(elec_date, desc(elec_date)), 
  ...
  labs(x="", y="", title= "Secon party when the winner is PSOE", subtitle = "total nº of votes per election in municipalites with more than 100.000 inhabitants") +
  theme_minimal() +
  theme(
...
ggplot(first_pp, aes(y = reorder(elec_date, desc(elec_date)), 
  ...
  theme_minimal() +
  labs(x="", y="", title= "Secon party when the winner is PP", subtitle = "total nº of votes per election in municipalites with more than 100.000 inhabitants") +
   theme(
    plot.title = element_text(face = "bold", size = 18, hjust = 0.5),
    plot.subtitle = element_text(face = "italic", size = 14, hjust = 0.5),
    legend.position = "bottom",
...
ggplot(error_table, aes(x = as.character(date_elec), y = total_error)) +
  ...
  labs(title = "Total Error by Election Date",
       x = "Election Date",
       y = "Total Error") 
```
...
ggplot(error_by_party, aes(x = siglas_2, y = total_error)) +
  geom_bar(stat = "identity", fill = "red" ) +
  labs(title = "Total Error by Party",
       x = "Party",
       y = "Total Error") 
```
...
```{r}
#error plot
ggplot(summarise_pollster_error, aes(y = avg_error, x = pollster)) +
  geom_point() +
